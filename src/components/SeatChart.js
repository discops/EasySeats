...
const SeatChart = ({ occasion, easyseats, provider, setToggle }) => {
  const [seatsTaken, setSeatsTaken] = useState(false)
  const [hasSold, setHasSold] = useState(false)

  const getSeatsTaken = async () => {
    const seatsTaken = await easyseats.getSeatsTaken(occasion.id)
  ...
  const buyHandler = async (_seat) => {
    ...
    const transaction = await easyseats.connect(signer).mint(occasion.id, _seat, { value: occasion.cost })
